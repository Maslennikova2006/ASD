...
TEST(TestUnsortedTableLLib, can_create_and_check_is_empty) {
    UnsortedTableL<int, std::string> table;
    EXPECT_TRUE(table.is_empty());
    table.insert(78, "Санкт-Петербург");
    EXPECT_FALSE(table.is_empty());
}
TEST(TestUnsortedTableLLib, check_insert) {
    UnsortedTableL<int, std::string> table;
    table.insert(52, "Нижний Новгород");
    EXPECT_FALSE(table.is_empty());
    ASSERT_NO_THROW(table.insert(97, "Москва"));
}
TEST(TestUnsortedTableLLib, check_for_insertion_exception) {
    UnsortedTableL<int, std::string> table;
    table.insert(52, "Нижний Новгород");
    ASSERT_ANY_THROW(table.insert(52, "Нижегородская область"));
}
TEST(TestUnsortedTableLLib, check_erase) {
    UnsortedTableL<int, std::string> table;
    table.insert(52, "Нижний Новгород");
    ASSERT_NO_THROW(table.erase(52));
    EXPECT_TRUE(table.is_empty());
}
TEST(TestUnsortedTableLLib, check_for_deletion_exception) {
    UnsortedTableL<int, std::string> table;
    table.insert(52, "Нижний Новгород");
    ASSERT_ANY_THROW(table.erase(25));
}
TEST(TestUnsortedTableLLib, check_found) {
    UnsortedTableL<int, std::string> table;
    table.insert(52, "Нижний Новгород");
